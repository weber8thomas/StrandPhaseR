...
bamregion2GRanges <- function(bamfile, bamindex = bamfile, region = NULL, pairedEndReads = FALSE, min.mapq = 10, filterAltAlign = TRUE) {
  ...
  if (pairedEndReads) {
    ...
    data.raw <- GenomicAlignments::readGAlignmentPairs(bamfile, index = bamindex, param = Rsamtools::ScanBamParam(tag = "XA", which = range(region), what = c("seq", "qual", "mapq", "cigar", "flag"), ))
  } else {
    ...
    data.raw <- GenomicAlignments::readGAlignments(bamfile, index = bamindex, param = Rsamtools::ScanBamParam(tag = "XA", which = range(region), what = c("seq", "qual", "mapq", "cigar"), flag = scanBamFlag(isDuplicate = F)))
  }
